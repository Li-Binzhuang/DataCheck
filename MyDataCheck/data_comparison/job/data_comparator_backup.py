...
def compare_two_files(
    ...
    print(f"\nSql文件主键列: 索引{sql_key_column} ({sql_key_name})")
    print(f"接口文件主键列: 索引{api_key_column} ({api_key_name})")
    ...
    # 构建接口文件的索引字典（以接口文件为基准）
    api_index = {}  # {key_value: (row_idx, row)}
    
    for row_idx, row in enumerate(rows_api):
        if api_key_column < len(row) and row[api_key_column] is not None:
            key_value = str(row[api_key_column]).strip()
            if key_value:
                api_index[key_value] = (row_idx, row)
    
    print(f"接口文件索引构建完成，共 {len(api_index)} 条记录")
    ...
    for i, header in enumerate(headers_api):
        ...
        if cust_no_idx_api is None and ("cust_no" in header_lower or "customer_no" in header_lower or "custno" in header_lower):
            cust_no_idx_api = i
            break
    ...
    for i, header in enumerate(headers_sql):
        header_lower = header.lower()
        if cust_no_idx_sql is None and ("cust_no" in header_lower or "customer_no" in header_lower or "custno" in header_lower):
            cust_no_idx_sql = i
            break
    
    # 输出找到的列信息
    if cust_no_idx_api is not None:
        print(f"找到接口文件cust_no列: 索引{cust_no_idx_api} ({headers_api[cust_no_idx_api]})")
    else:
        print(f"提示: 接口文件中未找到cust_no列")
    
    if cust_no_idx_sql is not None:
        print(f"找到Sql文件cust_no列: 索引{cust_no_idx_sql} ({headers_sql[cust_no_idx_sql]})")
    else:
        print(f"提示: Sql文件中未找到cust_no列")
    
    # 对比数据：遍历接口文件的每一行（以接口文件为基准）
    print(f"\n开始对比数据...")
    ...
    # 记录匹配的key值，用于后续找出仅在Sql文件中的数据
    matched_keys = set()
    
    for row_idx_api, row_api in enumerate(rows_api):
        if row_idx_api % 500 == 0:
        ...
        for row_sql in rows_sql:
            if sql_key_column < len(row_sql) and str(row_sql[sql_key_column]).strip() == key_value_api:
                sql_row = row_sql
                break
        ...
        # 获取主键值：优先从Sql文件获取，如果没有则从接口文件获取
        key_value = ""
        if sql_row and sql_key_column < len(sql_row) and sql_row[sql_key_column] is not None:
            key_value = str(sql_row[sql_key_column]).strip()
        
        # 如果Sql文件中没有主键值，使用接口文件的主键值
        if not key_value:
            key_value = key_value_api
        
        # 获取cust_no（优先从Sql文件获取，如果没有则从接口文件获取）
        cust_no = ""
        if cust_no_idx_sql is not None and sql_row and cust_no_idx_sql < len(sql_row):
            cust_no = str(sql_row[cust_no_idx_sql]).strip() if sql_row[cust_no_idx_sql] is not None else ""
        
        # 如果Sql文件中没有找到cust_no，尝试从接口文件获取
        ...
        for feature_name in all_features:
            api_idx, sql_idx = feature_mapping.get(feature_name, (None, None))
            ...
            # 获取Sql文件的值
            sql_value = ""
            if sql_idx is not None and sql_row and sql_idx < len(sql_row):
    ...
    for row_sql in rows_sql:
        if sql_key_column >= len(row_sql):
            continue
        
        key_value_sql = str(row_sql[sql_key_column]).strip() if row_sql[sql_key_column] is not None else ""
        
        if not key_value_sql:
            continue
        
        # 如果这个key在接口文件的索引中不存在，则记录为仅在Sql文件中的数据
