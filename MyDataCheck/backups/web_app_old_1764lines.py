...
data_comparison_job_dir = os.path.join(data_comparison_dir, "job")
...
compare_output_dir = os.path.join(output_data_dir, "data_comparison")
...
def execute_compare_flow(config: dict, output_queue: Queue):
    ...
    try:
        ...
        print(f"[INFO] Sql文件主键列: {config['key_column_1']}")
        print(f"[INFO] 接口文件主键列: {config['key_column_2']}")
        print(f"[INFO] Sql文件特征起始列: {config['feature_start_1']}")
        print(f"[INFO] 接口文件特征起始列: {config['feature_start_2']}")
        ...
        if not os.path.exists(file1_path):
            raise FileNotFoundError(f"Sql文件不存在: {file1_path}")
        if not os.path.exists(file2_path):
            raise FileNotFoundError(f"接口文件不存在: {file2_path}")
...
@app.route('/api/compare/config/save', methods=['POST'])
...
@app.route('/api/compare/config/load', methods=['GET'])
