-- name 用信42_sql取数_实时_国内
-- type StarRocks
-- author zhanglifeng703@hellobike.com
-- create time 2026-01-07 01:12:09
-- desc 
-- ============================================================================
-- 贷中特征衍生 - 海哥还款特征（修正版）
...
WITH base_loan_data AS (
    ...
    WHERE m0.use_create_time > m5.rp_create_time
...
, order_ranked AS (
    SELECT 
       ...
       ,loan_no
       ,loan_amt
       ,MIN(create_time) AS order_create_time
       -- 使用ROW_NUMBER按时间倒序排列，1=前第1笔（最近），2=前第2笔...
       ,ROW_NUMBER() OVER (PARTITION BY cust_no, use_credit_apply_id ORDER BY MIN(create_time) DESC) AS order_seq
    FROM base_loan_data
    GROUP BY cust_no, use_credit_apply_id, use_create_time, loan_no, loan_amt
)
...
, all_prev_repayments AS (
    SELECT 
       ...
       ,LAG(bld.settled_time) OVER (
           PARTITION BY orr.cust_no, orr.use_credit_apply_id, orr.use_create_time, orr.order_seq 
           ORDER BY bld.settled_time
       ) AS prev_settled_time
    FROM order_ranked orr
    INNER JOIN base_loan_data bld 
        ON orr.cust_no = bld.cust_no 
        AND orr.use_credit_apply_id = bld.use_credit_apply_id
        AND orr.loan_no = bld.loan_no                                           -- 用order_seq对应的loan_no关联
    WHERE orr.order_seq IN (1, 2, 3)                                            -- 只取前3笔订单
...
, repayment_features AS (
    SELECT 
       ...
       ,SUM(CASE WHEN day_of_week = 1 THEN 1 ELSE 0 END) AS on_weekend_cnt      -- 周末还款次数
       ,SUM(CASE WHEN day_of_week = 1 THEN per_settlement_amount ELSE 0 END) AS on_weekend_amount  -- 周末还款金额
    ...
    GROUP BY cust_no, use_credit_apply_id, use_create_time, prev_order_num
)
